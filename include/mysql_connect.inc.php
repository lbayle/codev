...
class SqlWrapper {
    ...
    */
   public function sql_free_result($request) {
      return mysql_free_result($request);
   }
 ...
 * @return resource For SELECT, SHOW, DESCRIBE, EXPLAIN and other statements returns a resource on success, or false on error.
 * For other type of SQL statements, INSERT, UPDATE, DELETE, DROP, etc, returns true on success or false on error.
 * @deprecated Use SqlWrapper::getInstance()->sql_query()
 */
function sql_query($query) {
   global $connection;
   return $connection->sql_query($query);
}

/**
 * Returns the text of the error message from previous MySQL operation
 * @return string the error text from the last MySQL function, or '' (empty string) if no error occurred.
 * @deprecated Use SqlWrapper::getInstance()->sql_error()
 */
function sql_error() {
   global $connection;
   return $connection->sql_error();
}
 ...
 * @param int $row The row number from the result that's being retrieved. Row numbers start at 0.
 * @return string The contents of one cell from a MySQL result set on success, or false on failure.
 * @deprecated Use SqlWrapper::getInstance()->sql_result()
 */
function sql_result($result, $row = 0) {
   global $connection;
   return $connection->sql_result($result, $row);
}
 ...
 * @param resource $result
 * @return object an object with string properties that correspond to the fetched row, or false if there are no more rows.
 * @deprecated Use SqlWrapper::getInstance()->sql_fetch_object()
 */
function sql_fetch_object($result) {
   global $connection;
   return $connection->sql_fetch_object($result);
}
 ...
 * @param resource $result
 * @return array an array of strings that corresponds to the fetched row, or false if there are no more rows.
 * @deprecated Use SqlWrapper::getInstance()->sql_fetch_array()
 */
function sql_fetch_array($result) {
   global $connection;
   return $connection->sql_fetch_array($result);
}
 ...
 * @param resource $result
 * @return array an associative array of strings that corresponds to the fetched row, or false if there are no more rows.
 * @deprecated Use SqlWrapper::getInstance()->sql_fetch_assoc()
 */
function sql_fetch_assoc($result) {
   global $connection;
   return $connection->sql_fetch_assoc($result);
}
 ...
 * @param string $unescaped_string The string that is to be escaped.
 * @return string the escaped string, or false on error.
 * @deprecated Use SqlWrapper::getInstance()->sql_real_escape_string()
 */
function sql_real_escape_string($unescaped_string) {
   global $connection;
   return $connection->sql_real_escape_string($unescaped_string);
}

/**
 * Get the ID generated in the last query
 * @return int The ID generated for an AUTO_INCREMENT column by the previous query on success, 0 if the previous query does not generate an AUTO_INCREMENT value, or false if no MySQL connection was established.
 * @deprecated Use SqlWrapper::getInstance()->sql_insert_id()
 */
 ...
 * @param resource $result
 * @return int The number of rows in a result set on success or false on failure.
 * @deprecated Use SqlWrapper::getInstance()->sql_num_rows()
 */
function sql_num_rows($result) {
   global $connection;
   return $connection->sql_num_rows($result);
}
 ...
 * @param resource $result
 * @return bool true on success or false on failure.
 * @deprecated Use SqlWrapper::getInstance()->sql_free_result()
 */
function sql_free_result($request) {
   global $connection;
   return $connection->sql_free_result($request);
}

/**
 * Close MySQL connection
 * @return bool true on success or false on failure.
 * @deprecated Use SqlWrapper::getInstance()->sql_close()
 */
function sql_close() {
   global $connection;
   return $connection->sql_close();
}

/**
 * Get the queries count
 * @return int Number of queries
 * @deprecated Use SqlWrapper::getInstance()->getQueriesCount()
 */
function getQueriesCount() {
   global $connection;
   return $connection->getQueriesCount();
}

/**
 * Get the connection link
 * @return mixed The link
 * @deprecated Use SqlWrapper::getInstance()->getLink()
 */
function getLink() {
   global $connection;
