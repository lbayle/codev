...
function rest_commands_get( \Slim\Http\Request $p_request, \Slim\Http\Response $p_response, array $p_args ) {
	...
	if(!is_null($command_id)) {
		...
		$testr= $_SESSION['userid']." username: ".$_SESSION['username']." realname:  ".$_SESSION['realname']."  teamid:  ".$_SESSION['teamid']." locale: ".$_SESSION['locale']."  projectid: ".$_SESSION['projectid'];
		$t_result = array( 'command' => $command->getId() . "    ".$testr );
	}
	else{
...
function rest_commands_delete( \Slim\Http\Request $p_request, \Slim\Http\Response $p_response, array $p_args ) {
	$command_id = isset( $p_args['id'] ) ? $p_args['id'] : $p_request->getParam( 'id' );
	...
	if(!is_null($command_id)) {
		...
		#}
		$command=new Command($command_id);
		if($command->delete($command_id)){
			$t_result = array( 'command ' => $command_id ." deleted" );
		}
		else{
			$t_result = array( 'command ' => $command_id ." not deleted" );
		}
	}
	else{
		$t_result="No command id found";
