   ...
   public function report_bug_form($event, $bug_id) {
      ...
      if (!$this->isAccessGranted($project_id)) {
         ...
         if (0 != count($cmdList)) {
            ...
            echo '<tr>';
            echo '<th class="category"><label for="codevtt_command_id">'.plugin_lang_get('command').'</label></th>';
            echo '<td><select multiple="multiple" id="codevtt_command_id" name="codevtt_command_id[]" size="'.$size.'">';
