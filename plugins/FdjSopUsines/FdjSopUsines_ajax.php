...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("FdjSopUsines_ajax");
   ...
   if('getFdjSopUsines' == $action) {

      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopUsines_startdate"));
      $endTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopUsines_enddate"));
      ...
      $indicator = new FdjSopUsines($pluginDataProvider);
      $indicator->execute();
      ...
      $html = $smartyHelper->fetch(FdjSopUsines::getSmartySubFilename());
      $data['fdjSopUsines_htmlTable'] = $html;
