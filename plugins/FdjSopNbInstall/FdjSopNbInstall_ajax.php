...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("FdjSopNbInstall_ajax");
   ...
   if('getFdjSopNbInstall' == $action) {
      ...
      $valueInUO = $attributesArray['valueInUO'];
      $userSettings = $attributesArray['userSettings'];
      //$logger->error(var_export($userSettings, true));

      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopNbInstall_startdate"));
      $endTimestamp   = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopNbInstall_enddate"));
      ...
      $indicator = new FdjSopNbInstall($pluginDataProvider);

      // override plugin default settings with current attributes
      $indicator->setPluginSettings(array(
          FdjSopNbInstall::OPTION_VALUE_IN_UO => $valueInUO,
          FdjSopNbInstall::OPTION_USER_SETTINGS => $userSettings,
      ));

      $indicator->execute();
      $data = $indicator->getSmartyVariablesForAjax();

      // construct the html content (FdjSopNbInstall_ajax.html)
      foreach ($data as $smartyKey => $smartyVariable) {
         $smartyHelper->assign($smartyKey, $smartyVariable);
         #$logger->debug("key $smartyKey = ".var_export($smartyVariable, true));
      }
      $html = $smartyHelper->fetch(FdjSopNbInstall::getSmartySubFilename());
      $data['fdjSopNbInstall_htmlContent'] = $html;

      // so that ajax can reload libraries (tabs, datatable)
      $data['fdjSopNbInstall_jsFiles'] = FdjSopNbInstall::getJsFiles();
