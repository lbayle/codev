...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("LoadHistoryIndicator_ajax");
   ...
   if('getLoadHistoryIndicator' == $action) {

      try {
      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("LoadHistoryIndicator_startdate"));
      $endTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("LoadHistoryIndicator_enddate"));
      ...
      $html = $smartyHelper->fetch(LoadHistoryIndicator::getSmartySubFilename());
      $data['LoadHistoryIndicator_htmlContent'] = $html;
