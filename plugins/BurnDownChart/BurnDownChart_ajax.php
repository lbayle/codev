...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("BurnDownChart_ajax");
   ...
   if('getBurnDownChart' == $action) {

      try {
      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("BurnDownChart_startdate"));
      $endTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("BurnDownChart_enddate"));
      ...
      $indicator = new BurnDownChart($pluginDataProvider);
      $indicator->setPluginSettings(array(
         BurnDownChart::OPTION_INTERVAL => $interval,
      ));
      ...
      $html = $smartyHelper->fetch(BurnDownChart::getSmartySubFilename());
      $data['BurnDownChart_htmlContent'] = $html;
