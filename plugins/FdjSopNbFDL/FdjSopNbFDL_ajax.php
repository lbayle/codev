...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("FdjSopNbFDL_ajax");
   ...
   if('getFdjSopNbFDL' == $action) {
      ...
      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopNbFDL_startdate"));
      $endTimestamp   = Tools::date2timestamp(Tools::getSecureGETStringValue("fdjSopNbFDL_enddate"));
      ...
      $indicator = new FdjSopNbFDL($pluginDataProvider);

      // override plugin default settings with current attributes
      $indicator->setPluginSettings(array(
          FdjSopNbFDL::OPTION_TASK_TYPE => $selectedTaskType,
          FdjSopNbFDL::OPTION_USER_SETTINGS => $userSettings,
      ));

      $indicator->execute();
      $data = $indicator->getSmartyVariablesForAjax();

      // construct the html content (FdjSopNbFDL_ajax.html)
      foreach ($data as $smartyKey => $smartyVariable) {
         $smartyHelper->assign($smartyKey, $smartyVariable);
         #$logger->debug("key $smartyKey = ".var_export($smartyVariable, true));
      }
      $html = $smartyHelper->fetch(FdjSopNbFDL::getSmartySubFilename());
      $data['fdjSopNbFDL_htmlContent'] = $html;

      // so that ajax can reload libraries (tabs, datatable)
      $data['fdjSopNbFDL_jsFiles'] = FdjSopNbFDL::getJsFiles();
