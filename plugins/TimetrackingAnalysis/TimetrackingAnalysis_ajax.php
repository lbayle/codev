...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("TimetrackingAnalysis_ajax");
   ...
   if('getTimetrackingAnalysis' == $action) {

      try {
      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("TimetrackingAnalysis_startdate"));
      $endTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("TimetrackingAnalysis_enddate"));
      ...
      $isOnlyActiveTeamMembers = ('on' !== $attributesArray[TimetrackingAnalysis::OPTION_IS_ONLY_TEAM_MEMBERS]) ? false : true;
      ...
      $indicator = new TimetrackingAnalysis($pluginDataProvider);
      $indicator->setPluginSettings(array(
         TimetrackingAnalysis::OPTION_INTERVAL => $interval,
         TimetrackingAnalysis::OPTION_IS_ONLY_TEAM_MEMBERS => $isOnlyActiveTeamMembers,
      ));
      ...
      $html = $smartyHelper->fetch(TimetrackingAnalysis::getSmartySubFilename());
      $data['TimetrackingAnalysis_htmlContent'] = $html;
