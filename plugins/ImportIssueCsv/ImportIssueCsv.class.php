...
/**
 * Description of ImportIssueCsv
 *
 * @author lob
 */
class ImportIssueCsv  extends IndicatorPluginAbstract {
   ...
   public static function getName() {
      return T_('Import issues');
   }
   public static function getDesc($isShortDesc = true) {
      return T_('Import a list of issues to MantisBT / CodevTT from a CSV file');
   }
      ...
      while (!$file->eof()) {
         while ($data = $file->fgetcsv($delimiter,$enclosure)) {
            ...
            if ('' != $data[0]) {
               ...
               $newIssue['wbsTree'] = Tools::convertToUTF8($data[6]);
               $issues[] = $newIssue;
            }
         }
      }
  ...
  /**
    ...
    */
    public function getSmartyVariables($isAjaxCall = false) {
        ...
        {
            $smartyVariables['importIssueCsv_projectid'] = $this->execData['projectid'];
            
            $smartyVariables['importIssueCs_teamName'] =  $this->execData['teamName'];
            $smartyVariables['importIssueCsv_projectName'] = $this->execData['projectName'];
            $smartyVariables['importIssueCsv_filename'] = $this->execData['filename'];
            
            $smartyVariables['importIssueCsv_commandList'] = SmartyTools::getSmartyArray($this->execData['commands'], 0);
            $smartyVariables['importIssueCsv_categoryList'] = SmartyTools::getSmartyArray($this->execData['projectCategories'], 0);
            $smartyVariables['importIssueCsv_targetversionList'] = SmartyTools::getSmartyArray($this->execData['projectTargetVersion'], 0);
            $smartyVariables['importIssueCsv_userList'] = SmartyTools::getSmartyArray($this->execData['activeMembers'], 0);
            ...
            $smartyVariables['importIssueCsv_newIssues'] = $this->execData['issues'];
        }
        
      
      if (array_key_exists('errorMsg', $this->execData)) {
         $smartyVariables['importIssueCsv_errorMsg'] = $this->execData['errorMsg'];
      ...
      if (false == $isAjaxCall) {
         $smartyVariables['importIssueCsv_ajaxFile'] = self::getSmartySubFilename();
      }
      $smartyVariables['importIssueCsv_ajaxPhpURL'] = self::getAjaxPhpURL();
