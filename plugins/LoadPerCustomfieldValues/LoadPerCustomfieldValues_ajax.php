...
if(Tools::isConnectedUser() && filter_input(INPUT_GET, 'action')) {
   ...
   $logger = Logger::getLogger("LoadPerCustomfieldValues_ajax");
   ...
   if('getLoadPerCustomfieldValues' == $action) {

      $startTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("LoadPerCustomfieldValues_startdate"));
      $endTimestamp = Tools::date2timestamp(Tools::getSecureGETStringValue("LoadPerCustomfieldValues_enddate"));
      ...
      $pluginDataProvider->setParam(PluginDataProviderInterface::PARAM_CUSTOMFIELD_ID, $selectedCustomfieldId);

      $indicator = new LoadPerCustomfieldValues($pluginDataProvider);
      ...
      $html = $smartyHelper->fetch(LoadPerCustomfieldValues::getSmartySubFilename());
      $data['LoadPerCustomfieldValues_htmlContent'] = $html;
